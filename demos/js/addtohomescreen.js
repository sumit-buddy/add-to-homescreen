...
(function(window, document, undefined) {
  ...
  platform.closePrompt = function() {
    var ath_wrapper = document.querySelector(options.athWrapper);

    if (ath_wrapper) {
      for (var i = 0; i < options.showClasses.length; i++) {
        ath_wrapper.classList.remove(options.showClasses[i]);
      }
  ...
  function _show() {
    if (canPrompt()) {
      if (_beforeInstallPrompt && !options.mustShowCustomPrompt) {
        triggerNativePrompt();
      } else {
        // fire the custom onShow event
        if (options.onShow) {
          options.onShow(platform);
        }
